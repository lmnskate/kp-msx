...
class Playable:
    def __init__(self, data):
        ...
        self.subtitles = list(SubtitleTrack(s) for s in data.get('subtitles', []))
    ...
    def _extract_video_url(data):
        ...
        if config.QUALITY is not None:
            video_files = list(
                i for i in data.get('files', []) if i['quality'] == config.QUALITY
            )
    ...
    def msx_action(self, proxy: bool = False, alternative_player: bool = False):
        ...
        return msx.play_action(
            self.video_url, proxy=proxy, alternative_player=alternative_player,
        )

    def msx_properties(self, proxy: bool = False, alternative_player: bool = False):
        props = {'resume:key': self.resume_key(), 'trigger:ready': self.trigger_ready()}
